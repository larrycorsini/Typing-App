...
// FIX: Import `allSoundPacks` directly.
...
const SettingsModal: React.FC<SettingsModalProps> = ({ onClose }) => {
    ...
    return (
        <div className="absolute inset-0 bg-slate-900/80 flex items-center justify-center z-30 backdrop-blur-sm animate-fadeIn" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="settings-title">
            <div ref={trapRef} className="card w-full max-w-lg animate-scaleIn" onClick={e => e.stopPropagation()}>
                                ...
                                return (
                                    <button
                                        ...
                                        className={`p-4 rounded-lg border-4 text-left transition-all disabled:opacity-50 disabled:cursor-not-allowed
                                            ${isActive ? 'border-[var(--dl-text)] bg-yellow-200' : 'border-slate-600 hover:border-cyan-500'}
                                        `}
                                    >
                                        <div className="font-bold text-lg">{pack.name}</div>
                                        {!isUnlocked && <div className="text-xs text-slate-500">Unlock via achievements</div>}
                                    </button>
                                )
                            })}
                        </div>
