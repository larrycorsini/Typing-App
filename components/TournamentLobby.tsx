...
import { GameState, Boss } from '../types';
...
const BossCard: React.FC<{ boss: Boss }> = ({ boss }) => {
    ...
    const canChallenge = meetsRunning && meetsSwimming && meetsFlying && !isDefeated;
    ...
    return (
        <div className={`card flex flex-col items-center gap-4 relative ${isDefeated ? 'opacity-50 bg-slate-900/50' : ''}`}>
            ...
            <div className="bg-slate-900/50 p-3 rounded-lg w-full text-left space-y-1">
                <p className="text-center text-slate-400 mb-2">WPM Target: <span className="font-bold text-white">{boss.wpm}</span></p>
...
const TournamentLobby: React.FC = () => {
    ...
    const bosses = characterService.bosses;

    return (
        <div className="w-full max-w-5xl">
            <div className="text-center mb-8">
                <h1 className="text-5xl font-bold text-cyan-400">Championship</h1>
                <p className="text-slate-400 text-lg mt-2">Defeat all the bosses to become the ultimate champion!</p>
            </div>
